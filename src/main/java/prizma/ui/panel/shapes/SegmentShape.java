...
public class SegmentShape {
    ...
    public SegmentShape(double x, double y, double width, double height, String name, Color color, DoubleSupplier zoomProvider) {
        this.baseColor = color;
        this.zoomProvider = zoomProvider;

        rect = new Rectangle(0, 0, width, height);
        rect.setFill(color.deriveColor(0, 1, 1, 0.2));
        rect.setStroke(color);
        rect.setStrokeWidth(2);
        rect.getStrokeDashArray().addAll(10.0, 5.0);
        rect.setCursor(Cursor.HAND);

        Label label = new Label(name);
        label.setLayoutX(5);
        label.setLayoutY(5);
        label.setStyle("-fx-font-size: 11px; -fx-font-style: italic; -fx-text-fill: " +
